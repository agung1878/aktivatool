...
function lionthemes_page_meta_box_callback( $post ) {
	...
	echo '<div class="bootstrap">';
		echo '<div class="option row">';
			...
			echo '<option value="fifth"'. (($header_val == 'fifth') ? ' selected="selected"' : '') .'>'. esc_html__('Header fifth', 'lionthemes') .'</option>';
			echo '<option value="sixth"'. (($header_val == 'fifth') ? ' selected="selected"' : '') .'>'. esc_html__('Header sixth', 'lionthemes') .'</option>';
			echo '</select></div>';
		echo '</div>';
		...
		echo '<div class="option row">';
			...
			echo '<option value="seventh"'. (($footer_val == 'seventh') ? ' selected="selected"' : '') .'>'. esc_html__('Footer seventh', 'lionthemes') .'</option>';
			echo '<option value="eight"'. (($footer_val == 'eight') ? ' selected="selected"' : '') .'>'. esc_html__('Footer eight', 'lionthemes') .'</option>';
			echo '</select></div>';
		echo '</div>';
...
//if(class_exists( 'WooCommerce' )){
	add_action( 'product_cat_add_form_fields', 'lionthemes_product_cat_add_new_meta_field', 10, 2 );
	add_action( 'product_cat_edit_form_fields', 'lionthemes_product_cat_edit_meta_field', 10, 2 );
	add_action( 'edited_product_cat', 'lionthemes_save_taxonomy_custom_meta', 10, 2 );  
	add_action( 'create_product_cat', 'lionthemes_create_taxonomy_custom_meta', 10, 2 );
//}  
//add field
function lionthemes_product_cat_add_new_meta_field() {
	?>
	<div class="form-field">
		<label for="cat_square_image"><?php echo esc_html__( 'Square image', 'lionthemes' ); ?></label>
		<input type="hidden" name="_square_image" id="lionthemes_cat_square_image" value="" />
		<div class="wp-media-buttons">
			<button id="lionthemes_square_image_media_button" class="button" type="button"/><?php echo esc_html__('Upload Image', 'lionthemes') ?></button>
			<button id="lionthemes_square_image_remove_media_button" class="button" type="button"><?php echo esc_html__('Remove', 'lionthemes') ?></button>
		</div>
		<div id="lionthemes_cat_selected_media"></div>
	</div>
<?php
}
// edit field
function lionthemes_product_cat_edit_meta_field($term) {
	$t_id = $term->term_id;
	$_square_image = get_term_meta($t_id, '_square_image'); 
	
	?>
	<tr class="form-field">
	<th scope="row" valign="top"><label for="cat_square_image"><?php echo esc_html__( 'Square image', 'lionthemes' ); ?></label></th>
		<td>
			<input type="hidden" name="_square_image" id="lionthemes_cat_square_image" value="<?php echo (!empty($_square_image[0])) ? esc_attr($_square_image[0]) : ''; ?>" />
			<div class="wp-media-buttons">
				<button id="lionthemes_square_image_media_button" class="button" type="button"/><?php echo esc_html__('Upload Image', 'lionthemes') ?></button>
				<button id="lionthemes_square_image_remove_media_button" class="button" type="button"><?php echo esc_html__('Remove', 'lionthemes') ?></button>
			</div>
			<div id="lionthemes_cat_selected_media"><?php if(!empty($_square_image[0])){ ?><img width="150" src="<?php echo esc_url($_square_image[0]) ?>" alt="" /><?php } ?></div>
		</td>
	</tr>
<?php
}
// Save field
function lionthemes_save_taxonomy_custom_meta( $term_id ) {
...
//add new
function lionthemes_create_taxonomy_custom_meta( $term_id ){
	...
	if(isset($_GET['taxonomy']) && $_GET['taxonomy'] == 'product_cat'){
		$cat_upload_js = '
			jQuery(document).ready(function($){
				var file_frame;
				$(document).on("click", "#lionthemes_square_image_remove_media_button", function(e){
					e.preventDefault();
					$("#lionthemes_cat_square_image").val("");
					$("#lionthemes_cat_selected_media").html("");
				});
				$(document).on("click", "#lionthemes_square_image_media_button", function(e){
					...
					file_frame.on("select", function(){
						...
						var url = attachment.url;
						var field = document.getElementById("lionthemes_cat_square_image");
						field.value = url;
						$("#lionthemes_cat_selected_media").html("<img width=\'150\' src=\'"+ url +"\' />");
						file_frame.close();
					});
