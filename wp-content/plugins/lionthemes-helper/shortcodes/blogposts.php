<?php
function homess_blogposts_shortcode( $atts ) {
	global $homess_opt;
	$post_index = 0;
	...
	extract($atts);
	if($image == 'wide'){
		$imagesize = 'homess-post-thumbwide';
	} elseif($image =='rectangle') {
		$imagesize = 'homess-post-thumbrectangle';
	}
	else {
		$imagesize = 'homess-post-thumb';
	}
	$html = '';
	...
	$html.='<div class="blog-posts'. esc_attr($row_cl).' ' .esc_attr($el_class).' ' .esc_attr($add_class).'">';
		...
		$html .= '<div class="inner-content '.  esc_attr($widget_style) .'">';
			...
			foreach ( $postslist as $post ) {
				...
				$class_nothumb = '';
				$diff_time = sprintf( _x( '%s ago', '%s = human-readable time difference', 'homess' ), human_time_diff( get_the_time( 'U', $post->ID ), current_time( 'timestamp' ) ) );
				...
				$html.='<div class="item-post post-'. $post->ID .' ' . $class_column . $class_nothumb . ' wow fadeInUp" data-wow-delay="'. esc_attr($duration) .'ms" data-wow-duration="0.5s">';
					$html.='<div class="post-wrapper">';
						...
						$html.='<div class="post-info">';	
							...
							$html .= '<div class="post-by"><span class="author">'. esc_html__('By', 'homess') .'</span><a class="author" href="'. get_author_posts_url($author_id) .'"> '. get_the_author_meta( 'user_nicename' , $author_id ) .'</a></div>';	
							$html .= '<div class="post-date"><span class="month"> '. get_the_date( 'F', $post->ID ) .'</span> <span class="day">'. get_the_date( 'd', $post->ID ) .'</span>, <span class="year"> '. get_the_date( 'Y', $post->ID ) .'</span></div>';					
							...
							$html.='<div class="post-excerpt">';
								$html.= wp_trim_words(strip_tags(get_post_field('post_content', $post->ID)), $length);
							$html.='</div>';
							$html.= '<a href="'. get_the_permalink($post->ID) .'"><span class="readmore-text">'. esc_html__('Read more', 'homess') .'</span></a>';
...
add_shortcode( 'blogposts', 'homess_blogposts_shortcode' );
?>