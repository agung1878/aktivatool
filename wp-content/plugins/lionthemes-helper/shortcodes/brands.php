<?php
function homess_brands_shortcode( $atts ) {
	global $homess_opt;
	$brand_index = 0;
	...
	if(empty($homess_opt[$brand_logos])) return;
	$brandfound = count($homess_opt[$brand_logos]);
	$owl = array(
		'data-owl="slide"',
	...
	if(!empty($homess_opt[$brand_logos])) { ?>
			<?php 
				echo $wrapdiv; 
				$duration = 0;
			?>
			<?php foreach($homess_opt[$brand_logos] as $brand) {
...
add_shortcode( 'ourbrands', 'homess_brands_shortcode' );
?>